...
{
    return [CMSpiralLayoutAttributes class];
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)path
{
    ...
    CGRectGetMidX(pageRect) - (_radius * (count-path.item) / denominator) * cosf(3 * path.item * M_PI /denominator),
    CGRectGetMidY(pageRect) + (_radius * (count-path.item) / denominator) * sinf(3 * path.item * M_PI /denominator)*1.5
                                    );
    //NSLog(@"%lf/%lf",(_radius * path.item / denominator) * cosf(3 * path.item * M_PI /denominator),(_radius * path.item / denominator) * sinf(3 * path.item * M_PI /denominator));
    
    BOOL iPad = [[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad;
    CGFloat scaleFactor = iPad? 1 : 0.5;
    CGFloat scale = (1.0-  0.75 * (path.item / denominator)) * scaleFactor;
...
{
    CMSpiralLayoutAttributes *attributes = [CMSpiralLayoutAttributes layoutAttributesForSupplementaryViewOfKind:kind withIndexPath:indexPath];
